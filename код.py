...
def search():
    ...
    if form.validate_on_submit():
        ...
        if d not in result:
            ...
            return redirect('/message', username, user2)
    return render_template('a.html', title='соцсеть.рф', form=form)


@app.route("/message", methods=['POST', 'GET'])
def index2(user_name, user_to_nmae):
    if request.method == 'GET':
        db_session.global_init("users3.db")
        db_sess = db_session.create_session()
        message = list(db_sess.query(Message).filter((Message.user_name.in_([user_name, user_to_nmae])),
                                                     (Message.user_to_name.in_([user_name, user_to_nmae]))))
        lst = []
        lst2 = []
        for i in message:
        ...
        if 'text' in w:
            if request.form['text'] is not None:
                mes = Message(content=request.form['text'], photo=None, user_name=user_name,
                              user_to_name=user_to_nmae)
                db_sess.add(mes)
                db_sess.commit()
        else:
            ...
            mes = Message(content='', photo=f'img/{name}', user_name=user_name,
                          user_to_name=user_to_nmae)
            db_sess.add(mes)
            db_sess.commit()
        message = list(db_sess.query(Message).filter((Message.user_name.in_([user_name, user_to_nmae])),
        ...
        if 'text' in w:
            if len(request.form['text']) > 0:
                return render_template("index23.html", message=reslst, user=user_name)
...
if __name__ == '__main__':
    app.run(port=8093, host='127.0.0.1')